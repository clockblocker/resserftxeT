...
async function getInfinitive(plugin: TextEaterPlugin, text: string) {
	const hebPartsSchema = z.array(
		z.array(z.object({ surf: z.string(), lem: z.string() })),
	);
	...
	const makeMdForHebParts = (parts: HebParts): string => {
		return parts
			.map((word) =>
				word.map((part) => `[[${part.lem}|${part.surf}]]`).join("|"),
			)
			.join(" ");
	};
